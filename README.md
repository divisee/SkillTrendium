...
Для каждой найденной вакансии выполняется запрос `GET /vacancies/{id}` к HH API (параллельными батчами). Из ответа извлекается полное описание (`description`), ключевые навыки (`key_skills`), профессиональные роли, зарплата, опыт, город. HTML-теги очищаются скриптом.

**6. Извлечение технологий (LLM, батчами по N)**
Очищенные вакансии группируются в **батчи по N штук** (размер батча настраивается; при тестировании использовалось N=50). Каждый батч отправляется в LLM с промптом на извлечение технологий. Модель возвращает JSON с нормализованными названиями (например, `JS → JavaScript`, `K8s → Kubernetes`).

**7. Retry-механизм**
После первичного извлечения система проверяет, для каких вакансий технологии не были извлечены. Такие вакансии автоматически отправляются на повторную обработку **меньшими батчами** с интервалом между запросами. Результаты мержатся с основными данными.
...
2. **Детальные описания:** Каждая вакансия обогащается полным описанием через `GET /vacancies/{id}`. Запросы идут параллельными батчами. Если детали недоступны — используются snippet из поисковой выдачи (fallback).

3. **Группировка для LLM:** Вакансии разбиваются на батчи по N штук (настраивается; при тестировании N=50). В каждый батч попадают ID, название и описание (до 1 500 символов) каждой вакансии. LLM получает все N вакансий в одном промпте и возвращает JSON с технологиями для каждого ID.

4. **Retry:** Вакансии с пустым списком технологий и описанием > 30 символов отправляются повторно меньшими батчами (с полным описанием до 2 000 символов).
...
- Ограничения API: 30 запросов в секунду. Параллельные запросы батчами укладываются в лимит
